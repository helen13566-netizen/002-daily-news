...
5. 기사 수 **목표는 AI 20~25건 / 종합 20~25건** (총 40~50). **각 섹션 하한 20건은 반드시 준수**하라.
6. **하한 20건 이하로 내려가는 것은 최후 수단**. 오직 **stream idle timeout 이 실제로 임박한 경우(이전 chunk 가 4분 넘게 지연)에만** 허용. 정상 속도로 진행 중이면 무조건 각 섹션 20건 이상 생성하라. 품질 핑계로 건수를 줄이지 말 것.
