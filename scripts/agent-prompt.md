...
5. 기사 수 **목표는 공식 AI 3~5건 / AI 20~25건 / 종합 20~25건 / 연예 8~12건** (총 51~67).
   ...
   - **연예 뉴스**(`category=entertainment_news` — 연합뉴스 연예 / 매일경제 연예 / 한국경제 연예) 는 중요한 순으로 **목표 10건** (8~12 범위). 가십·루머는 점수가 낮으니 자연스럽게 후순위가 된다.
...
##### 문체 규칙 (v22 — 중학생 가독성 강화)

**대상 독자: 중학생.** 한 번 읽고 이해 안 되면 실패. 아래 규칙을 **모든 인사이트 본문에 적용**하라.

- **한 문장 80자 이내** (60자 권장). 80자 넘기면 끊어 두 문장으로.
- **외래어·약자는 첫 등장 시 괄호로 풀이**: `IPO(주식 상장)`, `AGI(인공일반지능)`, `M&A(인수합병)`, `R&D(연구개발)`, `CPI(소비자물가)`, `M&A(인수합병)`, `락인(한 곳에 묶임)`.
- **한자어를 일상어로**:
  - "거버넌스" → "운영 방식" / "회사 운영 구조"
  - "재정의" → "다시 정함"
  - "잠재적" → "앞으로 가능성이 있는"
  ...
  - "자율성 보장" → "스스로 결정하게 함"
  - "엔터프라이즈" → "기업용"
- **숫자는 비교 단위 동반**: "130억달러" → "한국 돈 약 18조원, 삼성전자 1년 영업이익 절반".
...
- **구체 수치·회사명·연도·대응 수단을 최소 2개 포함**하라.
...
**전:** "1차 효과는 OpenAI가 AWS·구글클라우드와도 자유롭게 거래할 길이 열려 한국 기업의 클라우드 선택지가 넓어진다는 점입니다."

**후:** "1차 효과는 한국 기업이 쓸 수 있는 클라우드(데이터 보관·계산 서비스) 선택지가 넓어집니다. OpenAI가 아마존·구글 쪽과도 자유롭게 거래할 수 있게 됐기 때문입니다."
...
**Phase 3.5 — 연예 뉴스 2건씩 × 약 5 chunk** (v22 신규, `category=="entertainment_news"` 유지)

연예 pool 에서 중요 순으로 **목표 10건** (8~12)을 2건씩 나눠 처리. `/tmp/ent_chunk*.json` 으로 저장. AI 키워드 매치 여부와 무관하게 `category="entertainment_news"` 를 그대로 유지하라 (절대 ai_news/general_news 로 변경 금지).
...
- **연예 뉴스(`category=="entertainment_news"`) 목표 10건 (8~12 범위)**
- 총 51~70건이 되도록 선별
