...
##### 문체 규칙 (v22.1 — 중학생 가독성 강화: 금지어 + 강제 치환)

**대상 독자: 중학생.** 한 번 읽고 이해 안 되면 실패. 아래 규칙을 **모든 인사이트 본문(`extraction_reason` + `insights.*.text`) 에 강제 적용**하라.

###### 🚫 절대 사용 금지어 (raw 그대로 쓰지 마라 — 옆 컬럼대로 치환)
...
| 디스럽션 | 시장 흔들기 / 판 뒤집기 | |
| 의제 | 사회 토론 주제 | |
| 점화 | 다시 시작 | "다시 점화" → "다시 불붙다" |
| 관행 | 일하는 방식 / 오랜 습관 | |
| 보완 | 부족한 부분 채우기 | |
| 형평성 | 공평함 | |
| 카테고리 | 종류 / 분류 | 단 schema 필드명은 그대로 |
| 콘텐츠 | 작품 / 영상 | 맥락에 따라 |
| 마진 | 이익 폭 | |
| 옵션 | 선택지 | |

###### 🔤 외래어·약자 첫 등장 시 강제 괄호 풀이

| 약자 | 풀이 |
|------|------|
| IPO | 주식 상장 (회사가 주식을 일반 투자자에게 처음 파는 것) |
| AGI | 인공일반지능 (사람처럼 폭넓게 생각하는 AI) |
| M&A | 인수합병 (회사가 다른 회사를 사거나 합치는 것) |
| R&D | 연구개발 |
| CPI | 소비자물가 |
| GDP | 국내총생산 (한 나라의 1년 경제 규모) |
| OTT | 인터넷 영상 서비스 (넷플릭스·디즈니플러스 같은) |
| IP | 지식 재산 (책·영화·드라마 같은 작품 권리) |
| AGI/GAI/AGI | 인공일반지능 |
| ESG | 환경·사회·운영 책임 |
| LLM | 대형 언어 모델 (챗GPT 같은 AI) |
| API | 프로그램끼리 데이터 주고받는 통로 |
| FAANG | 미국 빅테크 5사 (페이스북·애플·아마존·넷플릭스·구글) |
| EU | 유럽연합 |

**위 표에 없는 외래어**(예: "오컬트", "캐스팅", "포트폴리오", "리스트럭처링" 등)도 처음 등장 시 **괄호로 한국어 풀이**를 반드시 붙여라. "오컬트(귀신·초자연 소재)", "캐스팅(배우 정하기)", "포트폴리오(투자 묶음)" 처럼.

###### 📏 문장·문단 규칙

- **한 문장 80자 이내** (60자 권장). 80자 넘기면 끊어 두 문장으로.
- **숫자는 비교 단위 동반**:
  - 큰 돈: "130억달러" → "한국 돈 약 18조원, 삼성전자 1년 영업이익 절반"
  - 시청률/지지율: "7%" → "100명 중 7명이 본 셈"
  - 기업 규모: "12조원 수출" → "한국 1년 김치 수출액의 약 50배"
- "→" 화살표 대신 서술형 문장 ("A가 일어나면 B가 됩니다").
- 능동태 위주.
- 각 인사이트 문단: **3~5문장, 200~350자**.
- **분석 구조를 text 에 드러내라** — "1차 효과는…, 2차 효과는…, 3차 효과는…" 식으로 **단계를 명시**.
- **구체 수치·회사명·연도·대응 수단을 최소 2개 포함**.

###### ✅ 자가 검수 (각 chunk 작성 후 즉시)

LLM 인 너 자신이 **chunk 결과 dict 작성 직후, 다음 항목을 머릿속으로 점검**하고 위반 발견 시 그 자리에서 다시 써라:

1. 위 금지어 표의 단어가 raw 그대로 들어갔는가? → 치환
2. 약자가 풀이 없이 처음 등장했는가? → 괄호 풀이 추가
3. 80자 넘는 문장이 있는가? → 두 문장으로 분리
4. 큰 숫자가 비교 단위 없이 나왔는가? → 비교 단위 추가

##### 전/후 예시

**전 (어려움):**
> OpenAI 거버넌스의 핵심 축인 마이크로소프트 관계 재정의는 산업 전반의 클라우드·모델 경쟁 구도를 바꿉니다.

**후 (쉬움):**
> OpenAI(챗GPT 만든 회사)와 마이크로소프트가 협력 구조를 새로 정합니다. 이 변화는 클라우드 시장과 AI 모델 경쟁 흐름을 바꿉니다.

**전:** "1차 효과는 미스터리·법정극 결합 드라마가 7%대로 마무리해 안정적 시청률을 확보한 점입니다."

**후:** "1차 효과는 추리·법정 드라마(법정에서 재판 다루는 작품) 가 7%대 시청률(100명 중 7명이 본 셈)로 안정권에서 마무리됐다는 점입니다."

**전:** "한국 연예계 표준 계약서 점검 요구가 다시 점화될 수 있습니다."

**후:** "한국 연예계 표준 계약서를 다시 손봐야 한다는 목소리에 다시 불이 붙을 수 있습니다."
...
| 연예 (별도 섹션) | 기본 C+D 적용. 단 외래어·전문용어 풀이 더 강하게 (대중 독자) |
| 스포츠·단순 속보·루머/가십 | insights 생략 (score ≤ 4, must_know 아님) |
