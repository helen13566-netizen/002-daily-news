...
**`insights` 필드는 `is_must_know=true` (relevance_score >= 8.0) 인 기사에만 작성**하라. must_know 아닌 기사는 `insights` 필드를 **생략** 한다 (작성 시간·LLM 응답 분량 절약). 독자(일반인·초등학생 수준 어휘)가 "뉴스 그 자체"를 넘어 **세상을 바라보는 판단 근육**을 기르도록 돕는 해석·전망 레이어다.
...
각 축의 text 를 작성할 때 아래 체크리스트의 **분석 구조를 text 에 드러내라**. 체크리스트 항목명(예: "Iceberg", "IMVAIN") 같은 **전문 용어는 text 에 직접 쓰지 말되**, 그 **틀의 결과(1차·2차·3차 효과, 확률 등급, 출처 약점 등)는 text 에 명시**하라. 초등학생 수준 어휘의 서술형 3~5문장, **200~350자**, 구체 수치·연도·회사명 2개 이상 포함.
