...
##### 시간 예산·Chunk 분할 (v10 — stream idle 방지)

**중요**: 50건 × 2~4축의 insights를 **단일 LLM 호출로 한꺼번에 생성하면 Anthropic API "Stream idle timeout" 에 걸린다**(이전 실행에서 확인됨). 반드시 아래 3단계로 쪼개 **각 Phase 를 독립된 bash 호출**로 실행하라. Phase 사이에 짧은 `echo "[progress] ..."` 로 stream 을 유지한다.

**Phase 1 — candidates 분류 + 기본 스코어링**
...
**Phase 2 — AI 뉴스만 insights 생성** (이 블록 **하나의 bash 호출**에서 최대 25건 분석)

```bash
python3 - <<'PY'
import json, pathlib
ai_pool = json.load(open("/tmp/ai_pool.json"))
# 너(LLM)의 추론으로 상위 20~25건 선별 + 각 기사의 insights dict 작성 (ripple/history 기본, 필요 시 bonus)
selected = [
  # {**article, "ai_summary": "...", "extraction_reason": "...", "relevance_score": ..., "is_must_know": ..., "insights": {...}},
  # ... 상위 20~25건
]
pathlib.Path("/tmp/ai_analyzed.json").write_text(json.dumps(selected, ensure_ascii=False), encoding="utf-8")
print(f"[progress] ai_analyzed: {len(selected)} done")
PY
```

**Phase 3 — 종합 뉴스만 insights 생성** (별도 bash 호출, 최대 25건)

```bash
python3 - <<'PY'
import json, pathlib
gen_pool = json.load(open("/tmp/gen_pool.json"))
selected = [
  # ... 상위 20~25건 종합 뉴스 분석 결과
]
pathlib.Path("/tmp/gen_analyzed.json").write_text(json.dumps(selected, ensure_ascii=False), encoding="utf-8")
print(f"[progress] gen_analyzed: {len(selected)} done")
PY
```

**Phase 4 — 합치기 + state/analyzed.json 작성**

```bash
python3 - <<'PY'
import json, pathlib
ai = json.load(open("/tmp/ai_analyzed.json"))
gen = json.load(open("/tmp/gen_analyzed.json"))
state = json.load(open("state/state.json"))
analyzed = {
    "issue_number": state["issue_number"],
    "generation_timestamp": state["current_generation_timestamp"],
    "trend_hashtags": [...],  # 너(LLM)가 ai+gen 에서 공통 주제 3~8개로 압축
    "articles": ai + gen,
}
pathlib.Path("state/analyzed.json").write_text(json.dumps(analyzed, ensure_ascii=False, indent=2), encoding="utf-8")
print(f"[progress] analyzed.json: {len(analyzed['articles'])} articles written")
PY
```

##### 분량 한도
- 한 기사당 insights 전체가 800자 이내 (유지).
- **`must_know=true` 기사는 반드시 insights 포함**. 그 외 기사도 가능한 포함하되 시간 촉박하면 생략 OK (render 가 자동 폴백).
- 각 Phase 분석 소요 시간: 5~8분 내 목표. 3~4분마다 `echo "[progress] ..."` 로 stream 유지.
