...
각 축의 text 를 작성하기 전에 반드시 **속으로** 아래 체크리스트를 수행하고, 결과 단서를 text 에 녹여라. **체크리스트 자체를 text 에 나열하지 말 것**. text 는 여전히 중학생 수준 어휘의 서술형 2~4문장, 120~220자.
...
**Phase 2 — AI 뉴스 3건씩 × 9 chunk** (각 chunk 가 **별도 bash 호출**)

이전 버전(10건·25건)은 Anthropic API "Stream idle timeout"에 걸렸다(4회 확인). 한 bash 호출(= 한 LLM 응답)이 너무 길면 tool call 직후 thinking 구간이 5분 idle 제한을 넘긴다. **반드시 3건씩만** 처리하고, 각 chunk 를 다음 chunk 전에 완전히 bash 호출로 끊어라.

반복 구조 (AI 뉴스 25건 → 3+3+3+3+3+3+3+3+1 = 9 chunk):

```bash
# Phase 2a — AI 뉴스 1~3번째 insights
python3 - <<'PY'
import json, pathlib
ai_pool = json.load(open("/tmp/ai_pool.json"))
# LLM이 생각한 3건 분석 결과 (ai_summary, extraction_reason, score, is_must_know, insights 포함)
selected = [ ... ]  # 3개
...
Phase 2b (4~6) → `/tmp/ai_chunk2.json`,
Phase 2c (7~9) → `/tmp/ai_chunk3.json`,
Phase 2d (10~12) → `/tmp/ai_chunk4.json`,
Phase 2e (13~15) → `/tmp/ai_chunk5.json`,
Phase 2f (16~18) → `/tmp/ai_chunk6.json`,
Phase 2g (19~21) → `/tmp/ai_chunk7.json`,
Phase 2h (22~24) → `/tmp/ai_chunk8.json`,
Phase 2i (25) → `/tmp/ai_chunk9.json` (나머지 1건).

**Phase 3 — 종합 뉴스 3건씩 × 9 chunk** (위와 동일 구조로 `/tmp/gen_chunk1.json` ~ `/tmp/gen_chunk9.json` 저장)
...
**총 20 bash 호출**(Phase 1 + AI 9 + 종합 9 + Phase 4) = 20 LLM 응답. 각 응답 3건 × 2~4축 × 150자 ≈ **최대 2,400자** 이하 → stream idle timeout 완전 회피.

**소요 시간**: 각 chunk 30-90초, 전체 20-40분 예상. sandbox session limit 경계선에 있을 수 있으니, 시간이 부족하면 AI 는 15건(5 chunk), 종합 15건(5 chunk) 수준에서 일찍 끊고 Phase 4로 진행해도 된다.
